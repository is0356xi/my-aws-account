...
```bash
Notification-StepFunctions$ sam build --use-container
```
...
```bash
Notification-StepFunctions$ sam logs -n StockCheckerFunction --stack-name Notification-StepFunctions --tail
```
...
```bash
Notification-StepFunctions$ pip install -r tests/requirements.txt --user
# unit test
Notification-StepFunctions$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
Notification-StepFunctions$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
aws cloudformation delete-stack --stack-name Notification-StepFunctions
```
